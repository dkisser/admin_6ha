...
public class TPZSService {
	...
	public static final OperErrCode 上架图片失败 = new OperErrCode("040601", "上架图片失败");
	public static final OperErrCode 驳回失败 = new OperErrCode("040602", "驳回失败");
	 ...
	 */
	public EasyuiDatagrid<VTp> getTPZSList(String sqr,String glr,List<Integer> lxList,int page,int rows) throws UnsupportedEncodingException{
	...
	public void revertDttp (String dm,String czr,String remark) throws OperException {
		try{
			...
			remarkSb.append("驳回，现处于").append(XWZT.二审未通过).append("状态，");
	 ...
	 */
	@Transactional(rollbackFor=Exception.class)
	public void groundDttpNews (String dm,String czr,String replaceDm) throws OperException {
		try{
			//先将原来的动态图片状态设置为已过期
			JTp old = new JTp();
			old.setDm(replaceDm);
			old = jTpDao.select(old);
			old.setZt(XWZT.已过期.getValue());
			jTpDao.updateByPrimaryKeySelective(old);
			//news动态图片修改状态
			JTp record = new JTp();
			record.setDm(dm);
			record = jTpDao.select(record);
			record.setZt(XWZT.上架中.getValue());
			jTpDao.updateByPrimaryKeySelective(record);
			...
			remarkSb_old.append("动态图片").append("（").append(dm).append("）");
			...
			e.printStackTrace();
			throw new OperException(上架图片失败);
		}
	 ...
	 * @return
	 */
	public EasyuiDatagrid<VTp> chooseTPList(Integer lx,int page,int rows){
		VTp tp = new VTp();
		tp.setLx(lx);
		tp.setZt(XWZT.上架中.getValue());
		List<VTp> newsList = vTpDao.selectList(tp);
		PageNavigator<VTp> pn = new PageNavigator<VTp>(newsList, rows);
		EasyuiDatagrid<VTp> datagrid = new EasyuiDatagrid<>(pn.getPage(page), newsList.size());
		return datagrid;
	}
	 ...
	 * old_zt:二审中
	 * new_zt:上架中
	 * remark:普通图片（<dm>）于<系统时间>经<czr>审批下架，现处于<当前状态>
	 * @param dm
	 * @throws OperException 
	 */
	@Transactional(rollbackFor=Exception.class)
	public void groundPttpNews (String dm,String czr,String replaceDm) throws OperException {
		try{
			//先将原来的动态图片状态设置为已过期
			JTp old = new JTp();
			old.setDm(replaceDm);
			...
			oldnews.setDate(new Date());
			StringBuilder remarkSb_old = new StringBuilder();
			remarkSb_old.append("普通图片").append("（").append(dm).append("）");
			remarkSb_old.append("于").append(DateUtils.toString(new Date())).append("经").append(czr);
			remarkSb_old.append("审批下架，现处于").append(XWZT.已过期);
			oldnews.setRemark(remarkSb_old.toString());
			lNewsDao.insertSelective(oldnews);
