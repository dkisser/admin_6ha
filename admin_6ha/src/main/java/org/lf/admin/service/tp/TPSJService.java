...
public class TPSJService {
	...
	public static final OperErrCode 新增普通图片失败 = new OperErrCode("040404", "新增普通图片失败");
	public static final OperErrCode 删除普通图片失败 = new OperErrCode("040405", "删除普通图片失败");
	public static final OperErrCode 上架普通图片失败 = new OperErrCode("040406", "上架普通图片失败");
	 ...
	 */
	@Transactional(rollbackFor=Exception.class)
	public void addPttp(HttpSession session,String czr,String title,String content,MultipartFile pic) throws OperException, IOException{
		JSONObject picObj = twsjService.uploadPic(pic, session, AdminProperties.PTTP_PIC_TARGDIR);
		ServletContext servletContext = session.getServletContext();
		String serverRealPath = servletContext.getRealPath("");
		String filePath = String.format(AdminProperties.PTTP_HTML_TARGDIR, sdf.format(new Date()));
		String fileName = picObj.getJSONObject("data").getString("picName").split("\\.jpg")[0]+".html";
		//生成对应的html文件
		AdminUtils.genHtml(content, title, serverRealPath+filePath, fileName);
		try {
			...
			lnews.setNewZt(XWZT.编辑中.getValue());
			StringBuilder remarkSb = new StringBuilder();
			remarkSb.append("普通图片").append("（").append(tp.getId().toString()).append("）");
			remarkSb.append("于").append(DateUtils.toString(new Date())).append("被").append(czr).append("提交，");
			remarkSb.append("现处于").append(XWZT.编辑中);
			lnews.setRemark(remarkSb.toString());
			lNewsDao.insertSelective(lnews);
		}catch (Exception e) {
			throw new OperException(新增普通图片失败);
		}
	}
	 ...
	 * @throws OperException 
	 */
	public void delPttp (Integer id) throws OperException {
		try {
			jTpDao.deleteByPrimaryKey(id);
		} catch (Exception e) {
			throw new OperException(删除普通图片失败);
		}
	}
	 ...
	 */
	@Transactional(rollbackFor=Exception.class)
	public void sjPttp (Integer id,String czr) throws OperException {
		try {
			//将zt改为一审中
			JTp tp = new JTp();
			tp.setId(id);
			...
			lnews.setNewZt(XWZT.一审中.getValue());
			StringBuilder resultSb = new StringBuilder();
			resultSb.append("普通图片（").append(id.toString()).append("）");
			resultSb.append("于").append(StringUtils.getCurrTime()).append("被").append(czr).append("提交，现处于").append(XWZT.一审中);
			lnews.setRemark(resultSb.toString());
			lNewsDao.insertSelective(lnews);
		} catch (Exception e) {
			e.printStackTrace();
			throw new OperException(上架普通图片失败);
		}
